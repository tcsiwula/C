...
# and print the result.  This uses core instructions.  Note that there
# are many ways to do this.  This is how the assembler handles it.
# 
	.text
	.globl	main
main:
	...
	move    $t1, $v0                # Put the input value in a safe
        ...
        slt     $t2, $t1, $t0           # If $t1 < $t0, $t2 = 1
                                        # Otherwise, $t2 = 0
        bne     $t2, $zero, gt_lab      # If $t2 != 0, go to gt_lab
                                # In other words, if $t2 = 1 go to gt_lab
                                # In other words, if $t1 < $t0 go to gt_lab
                                # In other words, if $t0 > $t1 go to gt_lab
