 ...
 * Homework Assignment 9 part 2
 * Purpose:  Implements a butterfly-structured global sum.
 *
 * Input:    None.
 * Output:   Random values generated by processes, and sum of random values.
 *
 * Compile:  mpicc -g -Wall -o global_sum_butterfly global_sum_butterfly.c
 * Run:      mpiexec -n <number of processes> ./global_sum_butterfly
 *
 * Notes:
...
int Global_sum(int my_contrib, int my_rank, int p, MPI_Comm comm)
{
    ...
    unsigned and_bit = 2;
    int my_slope;
    if((my_rank & and_bit) == 0) my_slope = 1;
    if((my_rank & and_bit) != 0) my_slope = 0;
    ...
    while (bitmask > 0)
    {
      ...
      printf("process %d, phase: %d, bitmask: %d, partner: %d, my_slope: %d.\n",
       my_rank, phase, bitmask, partner, my_slope);


      MPI_Sendrecv(&sum, 1, MPI_INT, partner, 0, &temp, 1, MPI_INT, partner, 0,
