/*
Storage is allocated correctly on the host and the device 2 / 2
The host input arrays are initialized from stdin, and the device arrays are initialized with cudaMemcpy 2 / 2
The kernel call has the right number of blocks and threads per block  2 / 2
The device is synchronized after the kernel call with cudaThreadSynchronize, and the result is copied to the host with cudaMemcpy 2 / 2
The result is printed on the host 2 / 2
Correct kernel implementation 4 / 4
